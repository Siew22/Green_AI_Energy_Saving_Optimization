...
class Fp32InputsTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        # 强制输入为FP32
        fp32_inputs = {k: v.to(torch.float32) if torch.is_tensor(v) and v.is_floating_point() else v for k, v in inputs.items()}
...
if __name__ == "__main__":
    ...
    qat_training_args = TrainingArguments(
        ...
        gradient_accumulation_steps=4, # Keep the effective batch size at 4
        ...
        #--- Ultimate fix: completely turn off all mixed precision related options ---
        fp16=False,          # Explicitly turn off FP16
        bf16=False,          # Explicitly turn off BF16
        tf32=False,          # Explicitly turn off TF32
        #amp_backend="no",    # Explicitly tell accelerate not to use any AMP backend
        # ---------------------------------------------------
        
        report_to="wandb",
        run_name="qat-finetune",
        # For 100% stability, temporarily remove the 8-bit optimizer
        # If you find OOM, add it back
        # optim="adamw_bnb_8bit" 
    )
    print("QAT TrainingArguments initialized successfully.")

    # --- Fix point 2 (required): Force conversion to FP32 before sending the model to Trainer ---
    ...
    try:
        ...
        # --- Save and Convert ---
        # After training is complete, the model is in qat_trainer.model
        ...
        # Save the state dictionary of the QAT-prepared model
        ...
        # Convert the model to the final quantized format
