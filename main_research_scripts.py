...
from transformers import BertForSequenceClassification, BertTokenizerFast, TrainingArguments, Trainer, AutoConfig
...
try:
    from utils import estimate_flops, measure_energy_and_speed, \
                      ...
                      manual_evaluate_model, MAX_SEQ_LENGTH # Import MAX_SEQ_LENGTH from utils
...
class QuantizableBert(nn.Module):
    def __init__(self, model):
        super(QuantizableBert, self).__init__()
        # QuantStub: 量化哨兵，将浮点数输入转换为量化张量
        self.quant = QuantStub()
        self.model = model
        # DeQuantStub: 反量化哨兵，将量化输出转换回浮点数
        self.dequant = DeQuantStub()

    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        # 1. 将输入张量通过 QuantStub 转换为量化格式
        # 我们只量化模型的输入 hidden_states，它是由 embedding 层产生的，所以我们先做 embedding
        embedding_output = self.model.bert.embeddings(
            input_ids=input_ids,
            token_type_ids=token_type_ids,
        )
        quantized_embedding_output = self.quant(embedding_output)

        # 2. 调用模型的 encoder 和后续层
        # Hugging Face 模型通常需要一个 extended_attention_mask
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=self.model.dtype)
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        
        # encoder 的输入是量化后的 embedding
        encoder_outputs = self.model.bert.encoder(
            quantized_embedding_output,
        ...
        # 3. 将最终的 logits 通过 DeQuantStub 转换回浮点数
        dequantized_logits = self.dequant(logits)

        # 4. 计算 loss (如果提供了 labels)
        loss = None
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss = loss_fct(dequantized_logits.view(-1, self.model.num_labels), labels.view(-1))
            
        # 5. 返回 Hugging Face 风格的输出
        from transformers.modeling_outputs import SequenceClassifierOutput
        return SequenceClassifierOutput(
            loss=loss,
            logits=dequantized_logits,
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 我们只进行最基础的格式设置，接受它会输出错误类型的 batch
    ...
    # --- Stage 4: Ultimate Mixed Precision Quantization (QAT) using Manual Loop ---
    print("\n--- Stage 4: Quantization-Aware Training (Manual PyTorch Loop) ---")
    
    # 0. 准备环境和数据 (确保数据集和评估用的dataloader准备好)
    print("Re-preparing dataset and dataloader for QAT...")
    # 假设 tokenized_datasets 和 eval_dataloader 仍然可用
    # 如果脚本是分步运行的，你可能需要在这里重新加载它们
    # train_dataset_for_qat = tokenized_datasets["train"]
    ...
    try:
        ...
        # 使用AutoConfig确保加载正确的模型结构
        pruned_config = AutoConfig.from_pretrained(final_pruned_save_path)
        model_to_quantize = BertForSequenceClassification.from_pretrained(
            final_pruned_save_path, 
            config=pruned_config
        ).cuda()
        print("Model loaded successfully for QAT.")
    except Exception as e:
        print(f"CRITICAL ERROR: Could not load model for QAT. Error: {e}")
        traceback.print_exc()
        sys.exit(1)

    # 2. 准备QAT模型
    print("\nWrapping the model for quantization...")
    quantizable_model = QuantizableBert(model_to_quantize)

    print("\nPreparing model for QAT...")
    try:
        quantizable_model.train()
        # 将包装后的模型送入 qconfig 函数
        model_with_qconfig = define_mixed_precision_qconfig_strategy(quantizable_model)
        model_prepared_for_qat = torch.quantization.prepare_qat(model_with_qconfig, inplace=False)
        model_prepared_for_qat.float().cuda()
    
        print("Model successfully prepared for QAT with FP32 enforcement.")
    except Exception as e:
        print(f"CRITICAL ERROR: Failed during model preparation for QAT. Error: {e}")
        traceback.print_exc()
        sys.exit(1)

    # 3. 设置手动训练循环的参数
    # 为了在你的6GB显存上稳定运行，使用极小的batch size
    QAT_BATCH_SIZE = 1 
    # 保持等效的数学批量大小
    QAT_GRAD_ACCUM_STEPS = EFFECTIVE_BATCH_SIZE // QAT_BATCH_SIZE 
    QAT_LEARNING_RATE = 1e-6 # QAT通常使用更小的学习率

    print(f"\nStarting QAT training with Pure PyTorch Loop...")
    print(f"  - Epochs: {NUM_EPOCHS_QAT}")
    print(f"  - Device Batch Size: {QAT_BATCH_SIZE}")
    print(f"  - Gradient Accumulation Steps: {QAT_GRAD_ACCUM_STEPS}")
    print(f"  - Effective Batch Size: {QAT_BATCH_SIZE * QAT_GRAD_ACCUM_STEPS}")
    print(f"  - Learning Rate: {QAT_LEARNING_RATE}")

    # 4. 创建QAT专用的数据加载器、优化器和学习率调度器
    ...
    optimizer = torch.optim.AdamW(model_prepared_for_qat.parameters(), lr=QAT_LEARNING_RATE)
    num_training_steps = (len(train_dataloader_qat) // QAT_GRAD_ACCUM_STEPS) * NUM_EPOCHS_QAT
    scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=0.0, total_iters=num_training_steps)
    
    print("\n--- Diagnosing DataLoader Output ---")
    try:
        # 获取第一个 batch 来检查它的内容
        first_batch = next(iter(train_dataloader_qat))
        print("Keys in the first batch from DataLoader:", list(first_batch.keys()))
        for key, value in first_batch.items():
            print(f"  - Key: '{key}', Type: {type(value)}, Shape/Value: {value.shape if isinstance(value, torch.Tensor) else value}")
    except Exception as e:
        print(f"Error while inspecting the first batch: {e}")
    ...
    for epoch in range(NUM_EPOCHS_QAT):
        ...
        progress_bar = tqdm(train_dataloader_qat, desc=f"QAT Epoch {epoch+1}")
    
        for step, batch in enumerate(progress_bar):
            # 1. 初始化一个空的输入字典
            inputs = {}
            # 【关键修复1】: 在这个循环中，batch_size 永远是1，因为我们在 DataLoader 中设置了它
            batch_size = 1 

            # 2. 【核心修复】: 手动处理并重塑每一个我们需要的键
            if 'input_ids' in batch and isinstance(batch['input_ids'], list):
                # 将张量列表连接，然后重塑为 (1, seq_length)
                inputs['input_ids'] = torch.cat(batch['input_ids']).view(batch_size, -1).cuda()
            elif 'input_ids' in batch and isinstance(batch['input_ids'], torch.Tensor):
                inputs['input_ids'] = batch['input_ids'].view(batch_size, -1).cuda()

            if 'attention_mask' in batch and isinstance(batch['attention_mask'], list):
                inputs['attention_mask'] = torch.cat(batch['attention_mask']).view(batch_size, -1).cuda()
            elif 'attention_mask' in batch and isinstance(batch['attention_mask'], torch.Tensor):
                inputs['attention_mask'] = batch['attention_mask'].view(batch_size, -1).cuda()

            if 'token_type_ids' in batch and isinstance(batch['token_type_ids'], list):
                inputs['token_type_ids'] = torch.cat(batch['token_type_ids']).view(batch_size, -1).cuda()
            elif 'token_type_ids' in batch and isinstance(batch['token_type_ids'], torch.Tensor):
                inputs['token_type_ids'] = batch['token_type_ids'].view(batch_size, -1).cuda()
            
            if 'label' in batch and isinstance(batch['label'], torch.Tensor):
                inputs['labels'] = batch['label'].cuda()
        
            # 3. 健壮性检查
            if not inputs or 'input_ids' not in inputs:
                print("Warning: Skipping a batch because 'input_ids' could not be constructed.")
                continue
            
            # 4. 移除不需要的 token_type_ids
            if 'token_type_ids' in inputs and not hasattr(model_prepared_for_qat.model.config, 'type_vocab_size'):
            ...
            # 6. 反向传播和优化器步骤
            if QAT_GRAD_ACCUM_STEPS > 1: loss = loss / QAT_GRAD_ACCUM_STEPS
            loss.backward()
            if (step + 1) % QAT_GRAD_ACCUM_STEPS == 0:
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad()
            progress_bar.set_postfix({"loss": loss.item() * QAT_GRAD_ACCUM_STEPS})
    ...
    try:
        ...
        # 保存最终的量化模型的状态字典
        quantized_model_path = f"{BASE_OUTPUT_DIR}/quantized_pruned_model_final.pt"
        torch.save(quantized_model.state_dict(), quantized_model_path)
        print(f"Final quantized model state_dict saved to {quantized_model_path}")
    
        # 创建一个字典来存储QAT阶段的结果，用于最终的报告
    ...
    # 清理内存
    del model_to_quantize, model_with_qconfig, model_prepared_for_qat, model_after_qat, quantized_model
    gc.collect()
    torch.cuda.empty_cache()
    ...
    # ====================================================================
    # --- 关键修复：在测量前，确保数据集是正确的PyTorch Tensor格式 ---
    # ====================================================================
    print("Ensuring evaluation dataset is in 'torch' format for measurement...")
    try:
        columns_to_keep = ["input_ids", "attention_mask", "label", "token_type_ids"]
        tokenized_datasets.set_format('torch', columns=columns_to_keep)
        # 重新创建dataloader以应用新格式
        eval_dataloader = DataLoader(tokenized_datasets["validation"], batch_size=BATCH_SIZE, drop_last=True)
    except:
        # 备用方案，以防token_type_ids不存在
    ...
    try:
        ...
        pruned_model_structure.cpu() # 确保它在CPU上
        print("  - Pruned structure rebuilt successfully.")

        # Step 2: 将重建的结构包装起来，为量化做准备
        quantizable_shell = QuantizableBert(pruned_model_structure)
        print("  - Model shell wrapped for quantization.")

        # Step 3: 应用QConfig并准备/转换模型，以获得最终的量化架构
        print("Step 2 & 3: Applying QConfig and converting to quantized format...")
        model_with_qconfig = define_mixed_precision_qconfig_strategy(quantizable_shell)
        model_with_qconfig.train() # 必须在 .train() 模式下调用 prepare_qat
        ...
        print("  - Structure converted to quantized format successfully.")

        # Step 4: 将保存的量化 state_dict 加载到我们新创建的量化“空壳”中
        ...
        # Step 5: 准备最终用于测量的模型（它现在是一个完整的、在CPU上的量化模型）
        quantized_model_loaded_for_measurement = quantized_model_shell
        quantized_model_loaded_for_measurement.eval() # 确保是评估模式
        print("\nSuccessfully loaded the custom quantized model for measurement (on CPU).")

        # --- Part 2: 在CPU上对加载好的模型进行测量 ---
        print("\nMeasuring quantized pruned model on CPU...")
        quantized_metrics = {} 
    
        # 调用测量函数，并明确传入 device="cpu"
        quantized_metrics_measurement = measure_energy_and_speed(
            quantized_model_loaded_for_measurement, 
            eval_dataloader, 
            description="Quantized Pruned Model Inference (CPU)",
            device="cpu"
        )
        quantized_metrics.update(quantized_metrics_measurement)

        # 调用评估函数，并明确传入 device="cpu"
        quantized_val_accuracy = manual_evaluate_model(
            quantized_model_loaded_for_measurement, 
            eval_dataloader, 
            description="Final Quantized Model Validation (CPU)",
            device="cpu"
        )
        quantized_metrics["accuracy"] = quantized_val_accuracy

        # --- Part 3: 估算FLOPs并合并最终结果 ---
        # 从剪枝后的模型中复用FLOPs和参数
        if 'pruned_metrics' in locals() and 'flops' in pruned_metrics:
            print("\nRe-using FLOPs and Params from the pruned model for the final report.")
            quantized_metrics["flops"] = pruned_metrics.get("flops")
            quantized_metrics["params"] = pruned_metrics.get("params")
        else:
            # 这是一个备用方案
            print("\nWarning: Could not retrieve FLOPs/Params from pruned_metrics. Estimating params only for the quantized model.")
            _, quantized_params = estimate_flops(quantized_model_loaded_for_measurement, MAX_SEQ_LENGTH)
            quantized_metrics["flops"] = 0
            quantized_metrics["params"] = quantized_params
    
        # 打印最终的参数和FLOPs
        if quantized_metrics.get("params") is not None:
            print(f"Quantized Model Params: {quantized_metrics['params']:,}")
        if quantized_metrics.get("flops") is not None:
            print(f"Quantized Model FLOPs: {quantized_metrics['flops']/1e9:.2f} GFLOPs (Theoretical, from pruned structure)")

        # 将所有测量到的、最准确的数据更新到 qat_metrics 字典，为Stage 6做准备
        if 'qat_metrics' in locals():
            qat_metrics.update(quantized_metrics)
        else:
            qat_metrics = quantized_metrics
        print("\nStage 5 measurement complete.")

    # except 块用于捕捉任何加载或测量过程中的异常
    except Exception as e:
        print(f"An unexpected error occurred during Stage 5: {e}")
        traceback.print_exc()
        sys.exit(1)
