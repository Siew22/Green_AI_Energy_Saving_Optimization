...
def estimate_flops(model, max_seq_length=MAX_SEQ_LENGTH):
    ...
    try:
        # --- 最终的、最可靠的调用方式 ---
        # 我们不再依赖thop自动解包字典，而是手动将tensor作为独立的参数传入。
        # 这是一个更底层的调用方式，兼容性最好。
        
        forward_args = model_copy.forward.__code__.co_varnames
        
        # 根据模型的forward签名，构建输入元组
        inputs_tuple = ()
...
def measure_energy_and_speed(model, dataloader, description="Measuring Model", device="cuda"):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=description):
            inputs = {}
            # 首先确定批量大小
            # batch.get('label', ...) 确保即使没有label也能工作
            batch_size = batch.get('label', batch.get('input_ids', [])).size(0)
            ...
            # 如果batch为空或没有input_ids，则跳过
            if not inputs or 'input_ids' not in inputs:
                print(f"Warning: Skipping a batch in '{description}' due to missing 'input_ids'.")
                continue
            
            # 移除模型不需要的token_type_ids
            # 这个检查可以让你的代码兼容DistilBERT等模型
            config_obj = model.model.config if hasattr(model, 'model') else model.config # 兼容包装后的模型
            if 'token_type_ids' in inputs and not hasattr(config_obj, 'type_vocab_size'):
                del inputs['token_type_ids']
...
def define_mixed_precision_qconfig_strategy(model):
    """
    Defines the quantization configuration. 
    This version is specifically designed to work with the QuantizableBert wrapper.
    """
    # 你的函数签名和第一行代码是正确的
    print("Applying mixed precision quantization strategy to the wrapped model...")
    model_with_qconfig = copy.deepcopy(model)
    qconfig_int8 = torch.quantization.get_default_qat_qconfig('fbgemm')
    
    # 【【【关键修复点】】】
    # 我们遍历的是 model_with_qconfig.model.named_modules()
    # 而不是 model_with_qconfig.named_modules()
    for name, module in model_with_qconfig.model.named_modules():
         # 下面的逻辑保持不变，因为它们现在能正确地找到 nn.Linear 等层了
         if isinstance(module, nn.Linear):
              ...
              # Embeddings and LayerNorm are typically kept in FP32
              module.qconfig = None

    print("Qconfig applied to the inner model layers successfully.")
